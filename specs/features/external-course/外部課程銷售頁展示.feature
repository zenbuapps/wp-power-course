...
Feature: 外部課程銷售頁展示
  ...
  # Issue #219：外部課程的「課程資訊」統計項（開課時間 / 課程時長 / 章節數量 /
  # 觀看時間 / 學員人數 / 全數上架）對外部課程而言沒有真實資料，過去一律塞「-」
  # 佔位符顯示，造成前台一定出現一整排橫線。站長拍板：外部課程不做任何開關，
  # 直接整塊（含「課程資訊」標題）隱藏。站內課程維持原本逐項 show_* 開關，不受影響。

  Rule: 銷售頁 Body - 外部課程完全隱藏「課程資訊」整塊區域（含標題）

    Example: 外部課程銷售頁不出現課程資訊區塊
      When 訪客進入外部課程 200 的銷售頁
      Then 不應看到「課程資訊」區塊標題
      And 不應看到任何課程資訊統計項
      And 頁面不應出現任何 "-" 佔位符

    Example: 外部課程即使殘留站內顯示設定仍隱藏課程資訊
      Given 外部課程 200 殘留 show_total_student 為 "yes" 的舊設定
      When 訪客進入外部課程 200 的銷售頁
      Then 不應看到「課程資訊」區塊標題
      And 不應看到「學員人數」統計項

  Rule: 銷售頁 Body - 站內課程「課程資訊」區塊行為不受 Issue #219 影響

    Example: 站內課程仍依逐項 show_* 開關顯示課程資訊
      When 訪客進入站內課程 100 的銷售頁
      Then 應看到「課程資訊」區塊標題
      And 應依各統計項的 show_* 設定顯示對應項目
