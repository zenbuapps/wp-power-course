...
Feature: 重新計算課程已售出數量
  ...
  # - Q7 (C)：兩種觸發方式並存——
  #           (1) plugin 升級到含此修正的版本時，自動以 Action Scheduler 背景跑一次（分批避免 timeout）；
  #           (2) 設定頁保留「重新計算所有課程已售出數量」手動按鈕，供日後排查或誤判時重算。
  ...
  Rule: 前置（權限）- 僅管理員可手動觸發重新計算
    ...
    Example: 非管理員觸發重新計算失敗
      When 用戶 "Alice" 觸發重新計算所有課程已售出數量
      Then 操作失敗，錯誤為「權限不足」
  ...
  Rule: 後置（狀態）- 重新計算以有效訂單重建 total_sales，修正虛高數據
    ...
    Example: 重新計算後虛高數據被修正為有效訂單數
      ...
      When 管理員 "Admin" 觸發重新計算所有課程已售出數量
      Then 操作成功
      And 課程 100 的 total_sales 應為 1
    ...
    Example: 重新計算為冪等操作，重複執行結果一致
      Given 系統中存在以下訂單：
        ...
        | ORDER-A  | Alice | 500     | completed |
      When 管理員 "Admin" 觸發重新計算所有課程已售出數量
      And 管理員 "Admin" 再次觸發重新計算所有課程已售出數量
