<?php
/**
 * MCP Report Student Count Tool
 *
 * 查詢指定日期區間內的學員數量統計（以 course_granted_at 為新增時間）。
...
/**
 * Class ReportStudentCountTool
 *
 * 對應 MCP ability：power-course/report_student_count
 */
final class ReportStudentCountTool extends AbstractTool {
	...
	public function get_description(): string {
		return \__( '查詢指定日期區間內新加入課程的學員數量統計（按 interval 分組）。區間上限為 365 天。', 'power-course' );
	}
	...
	public function get_output_schema(): array {
		return [
			...
			'properties' => [
				'total'     => [
					'type'        => 'integer',
					'description' => \__( '區間內的學員總數。', 'power-course' ),
				],
				'intervals' => [
					'type'        => 'array',
	 ...
	 * @return array{total: int, intervals: array<int, array{time_interval: string, count: int}>}|\WP_Error
	 */
	protected function execute( array $args ): array|\WP_Error {
		...
		if ( ! empty( $args['product_includes'] ) && \is_array( $args['product_includes'] ) ) {
			$service_args['product_includes'] = array_values(
				array_filter(
		...
		try {
			$result = Stats::get_student_count_stats( $service_args );
		} catch ( \Throwable $th ) {
			...
			return new \WP_Error(
				'mcp_report_student_count_failed',
				$th->getMessage(),
				[ 'status' => 500 ]
			);
		}
